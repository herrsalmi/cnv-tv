...
gc.norm <- function(ref, cvg, method = "lowess"){
  window.size <- 100
...
## old stuff ###########################################
window.size <- 101
gc <- rowSums(letterFrequencyInSlidingView(ref[[1]], window.size, c("G","C")))/window.size
pad.right <- function (rle, value=0, len) {
  if (length(rle) > len) stop("length mismatch: len must be >= length(rle)!")
  if (len > length(rle))
    c(rle, Rle(value, len-length(rle)))
...
d <- median(cvgP[min(nonZeroIndex):length(cvgP)])
